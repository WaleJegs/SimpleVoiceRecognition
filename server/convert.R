...
conversion <- function(input){
  ...
  pr <- spec(readWave('/Users/walejegede/Downloads/cozywaved93overload.wav'))
  pr <- specprop(pr, str = FALSE, mel = FALSE, plot = FALSE)
  df <- as.data.frame(dfreq(readWave('/Users/walejegede/Downloads/cozywaved93overload.wav')))
  free <- as.data.frame(spectral_entropy(spec(readWave('/Users/walejegede/Downloads/cozywaved93overload.wav'))))
  fundy <- as.data.frame(fund(readWave('/Users/walejegede/Downloads/cozywaved93overload.wav'), plot = FALSE))
  fundy <- na.omit(fundy)
  pr$meanfreq <- pr$mean
  pr$skew <- pr$skewness
  pr$kurt <- pr$kurtosis
